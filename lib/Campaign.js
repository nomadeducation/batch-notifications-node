...
/**
 * This utility function will convert string representation of dates into Date objects
 *
 * @param {CampaignInfos} campaignInfos
 * @returns {CampaignInfos}
 */
function convertStringToDate (campaignInfos) {
    campaignInfos.created_date = new Date(campaignInfos.created_date);

    if (campaignInfos.local_push_time) {
        campaignInfos.local_push_time = new Date(campaignInfos.local_push_time);
    } else {
        campaignInfos.push_time = new Date(campaignInfos.push_time);
    }
...
class Campaign {
    ...
    get (token) {
        ...
        .then(response => convertStringToDate(response.data));
    }
    ...
    list (from = 0, limit = 10, live, fromAPI) {
        ...
        .then(response => {
            ...
            if (!Array.isArray(detailsList)) {
                return Promise.reject(new Error("the response is malformed. Reason: the data must be an array"));
            }

            return detailsList.map(details => convertStringToDate(details));
        });
    }
    ...
    stats (token) {
        ...
        .then(function (response) {
            ...
            if (!Array.isArray(details)) {
                return Promise.reject(new Error(`the response is malformed. Reason: the "${detailKey}" key must be an array`));
            }
