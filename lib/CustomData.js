...
class CustomData {
    ...
    postSpecific (userId, payload) {
        ...
        const validate = ajv.compile(parameters.postSpecificMinimal);
    ...
    postBulk (payload) {
        // don't send the payload if it's malformed
        const validate = ajv.compile(parameters.postBulkMinimal);
        const isValid = validate(payload);
     ...
     * @returns {Promise.<String, Error>} get the custom data token if fulfilled
     */
    deleteSpecific (userId, payload) {
        return this.request.delete(`/data/users/${userId}`, {data: payload})
        .then(function (response) {
            const tokenKey = "token";
